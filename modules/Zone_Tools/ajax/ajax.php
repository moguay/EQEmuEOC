<?php
	...
	if($_GET['ZoneID'] && $_GET['Selector'] == 1){
		...
		$QueryResult = mysqli_query($Query, $db); $zone_info = array();
	...
	if($_GET['ZoneID'] && isset($_GET['VersionToDelete']) && $_GET['Submit'] == 1){
		if($_GET['DeleteType'] == "partial"){
			...
			mysqli_query("DELETE FROM `spawn2` WHERE `zone` = '". $ZoneSN . "' AND `version` = '". $_GET['VersionToDelete'] . "';", $db);
		...
		if($_GET['DeleteType'] == "full"){
			...
			# mysqli_query("DELETE FROM grid WHERE id='999999999999' (". $QueryAdd1 . ") AND ;", $db);
			# mysqli_query("DELETE FROM grid_entries WHERE gridid='999999999999' (". $QueryAdd2 . ");", $db);
			...
			$QueryResult = mysqli_query($Query, $db);
			while($row = mysqli_fetch_array($QueryResult)){
				mysqli_query("DELETE FROM spawnentry WHERE spawngroupID='" . $row['spawngroupID'] . "';", $db);
				mysqli_query("DELETE FROM spawngroup WHERE id='" . $row['spawngroupID'] . "';", $db);
				mysqli_query("DELETE FROM npc_types WHERE id='" . $row['npcID'] . "' AND id > 999;", $db);
			...
			mysqli_query("DELETE FROM spawn2 WHERE zone='" . $ZoneSN . "' AND  spawn2.version='" . $_GET['VersionToDelete'] . "';", $db);
			$query_total_output .= "DELETE FROM spawn2 WHERE zone='" . $ZoneSN . "' AND  spawn2.version='" . $_GET['VersionToDelete'] . "';" . "\n"; 
		}
		if($_GET['ObjectsDelete'] == 1){ 
			mysqli_query("DELETE FROM object WHERE zoneid = " . $_GET['ZoneID'] . " AND version = ". $_GET['VersionToDelete'] . ";", $db);
			$query_total_output .= "DELETE FROM object WHERE zoneid = " . $_GET['ZoneID'] . " AND version = ". $_GET['VersionToDelete'] . ";" . "\n"; 			
		}
		if($_GET['DoorsDelete'] == 1){ 
			mysqli_query("DELETE FROM doors WHERE zone = '" . $ZoneSN . "' AND version = " . $_GET['VersionToDelete'] . ";", $db); 
	...
	if($_GET['ZoneID'] && $_GET['CopyToolSubmit']){
		...
		$result = mysqli_query('SELECT MAX(id) + 1 AS id FROM `spawn2`', $db);
		...
		if($_GET['NPCDATA'] == "existing"){
			...
			if($_GET['NPCGRIDS']){
				/* Grids */
				$result = mysqli_query('SELECT MAX(id) + 1 AS id FROM `grid` WHERE `zoneid` = "'. $_GET['ZoneID'] . '"', $db);
				$row = mysqli_fetch_assoc($result);
				$ID = $row['id'];
			...
			/* npc_types */
			$result = mysqli_query('SELECT MAX(id) + 1 AS id FROM `npc_types`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
			$Query = "SELECT
				npc_types.*
			...
			/* spawngroup */
			$result = mysqli_query('SELECT MAX(id) + 1 AS id FROM `spawngroup`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
			$Query = "SELECT
				spawngroup.*
	...
	if($_GET['ZoneID'] && $_GET['ImportTool'] && isset($_GET['Source']) && isset($_GET['Dest'])){
		...
		if($_GET['Doors'] == 1){
			...
			/* Doors */
			$result = mysqli_query('SELECT MAX(id) + 1 AS id FROM `doors`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
			$result = mysqli_query('SELECT MAX(doorid) + 1 AS id FROM `doors` WHERE `zone` = "' . $ZoneSN .'" AND (version = ' . $_GET['Source'] . ' or version = -1);', $db);
			$row = mysqli_fetch_assoc($result);
			$DoorID = $row['doorid'];
		...
		if($_GET['Objects'] == 1){
			...
			/* Objects */
			$result = mysqli_query('SELECT MAX(id) + 1 AS id FROM `object`', $db);
			$row = mysqli_fetch_assoc($result);
			$ID = $row['id'];
		...
		while ($row = mysqli_fetch_array($QueryResult)) { 
			...
			$ID++;
			$Query = substr($InsertQuery, 0, -2) . ') VALUES ('. substr($InsertQuery2, 0, -2) . ');';
			mysqli_query($Query, $db); # if(mysqli_error()){ echo '<b style="color:red">' . mysqli_error() . '</b><br>'; }
			$query_total_output .= $Query . "\n"; 
			#echo $Query . '<br>';
		}
